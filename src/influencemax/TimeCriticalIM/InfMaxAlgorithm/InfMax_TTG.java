 ...
 */
public class InfMax_TTG implements InfMaxInterface {
    ...
    public static void main(String[] args){
        //假设25min
        InfMax_TTG test = new InfMax_TTG(36400, 1500, 1.0/ 320.0, "");
...
//        System.out.println("影响力模拟值为：" + curTotalInf);
        ...
        double maxDelay = 4000;//TODO 2018/04/27 为了测试yita的值，1/320，这里试试更长的时间
        double theta = 1.0/3.2;
        for(int i = 1; i < 5; i++){
            theta = theta / 10;
            InfMaxInterface myAlgorithm = new InfMax_TTG(startTimeSecond, maxDelay, theta, "");
            long startTime = System.currentTimeMillis();
            HashSet<Long> curSeed = myAlgorithm.getKSeeds(20);
            long endTime   = System.currentTimeMillis(); //程序结束记录时间
            long totalTime = endTime - startTime;       //总消耗时间
            MontoCarloClass monto = new MontoCarloClass(curSeed, startTimeSecond, maxDelay, 2, "");//TODOdone
            double infVal = monto.getMontoCarloInf();
            System.out.println("theta值：===1/" + (3.2 * Math.pow(10, i)) + "种子个数:" + 20 + "======" + myAlgorithm.getName() + "======影响力值======" + infVal + "======耗时======" + (totalTime) + "ms");
        }
     ...
     * @param theta 影响传播阈值
     * @param tableNameSuffix 表示要查询的表的后缀
     */
    public InfMax_TTG(double startTime, double maxDelay, double theta, String tableNameSuffix){
        this.m_maxDelay = maxDelay;
        this.m_theta = theta;
        this.m_startTimeSecond = startTime;
        m_tableNameSuffix = tableNameSuffix;
    }
    public InfMax_TTG(double maxDelay, double[] qDist){
        this.m_maxDelay = maxDelay;
    }
    ...
    public HashSet<Long> getKSeeds(int k){
        ...
        //infList里存储的是节点的影响力或边际影响力，会每轮逐渐递减，所以只是作为顺序的参考，不需要每次选择种子节点都还原。可以加速计算。
        ...
        for(int i = 0; i < k; i++){
            ...
            for(int j = 0; j < infList.size(); j++){
                //这里把影响力最大的一个除掉
                if(infList.get(j).uid == 25073877){//TODO 这里只是为了yita的设置临时加的，需要去掉
                    continue;
                }
